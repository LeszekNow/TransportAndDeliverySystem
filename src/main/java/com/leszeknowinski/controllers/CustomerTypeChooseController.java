...
public class CustomerTypeChooseController implements Initializable {
    ...
    public void executeCustomerAdding()throws Exception{
        ...
        if(customer.equals("Customer")) {
            ((Stage) select.getScene().getWindow()).setScene(controllersHelper.loadFXMLScreen("CustomerAddingScreen.fxml"));
        }
        else if(customer.equals("Customer B2B / VAT")) {
            ((Stage) select.getScene().getWindow()).setScene(controllersHelper.loadFXMLScreen("CustomerB2BAddingScreen.fxml"));
        }
    }
