...
public class DriverOrderFinishController {
    ...
    public void finishOrder() {
        dbHandler.connectToDataBase("UPDATE torder SET inProgress = false, completed = true WHERE driverId = " + dbHandler.getSthIdFromDB("SELECT id FROM tdriver WHERE username = '" + username.getText() + "';"));
        int driverId = dbHandler.getSthIdFromDB("SELECT id FROM tdriver WHERE username = '" + username.getText() + "';");
