...
public class CustomerTypeChooseController3 implements Initializable {
    ...
    public void executeRegistration()throws Exception{
        ...
        if(customer.equals("Customer")) {
            ((Stage) select.getScene().getWindow()).setScene(controllersHelper.loadFXMLScreen("CustomerRegistrationScreen.fxml"));
        }
        else if(customer.equals("Customer B2B / VAT")) {
            ((Stage) select.getScene().getWindow()).setScene(controllersHelper.loadFXMLScreen("CustomerB2BRegistrationScreen.fxml"));
        }
    }
